...
#include "rmx_receive_mem_alloc_rgstr.h"
...
ReturnStatus RmxReceiveMemAllocRgstr::operator()()
{
    ...
    status = rmx_register_memory(input_mem_buffer, &mem_registry);
    ...
    status = rmx_deregister_memory(input_mem_buffer, &device_iface);
...
{
    return RmxReceiveMemAllocRgstr().run(argc, argv);
}
